<template>
    ...
    >
        ...
        <div class="hidden items-center gap-4 md:flex lg:gap-6" style="-webkit-app-region: no-drag">
            ...
            <ConnectionStatus />
            <span
                v-if="captureMode === 'recall' && captureStatus"
                class="min-w-20 text-xs font-medium text-gray-600 dark:text-gray-300"
                aria-live="polite"
            >
                Teams: {{ captureStatusLabel }}
            </span>
            ...
            <template v-if="captureMode === 'local'">
...
import { shouldRequestLocalPermissions } from '@/composables/useMeetingCaptureOrchestrator';
...
const captureStatusLabel = computed(() => {
    const labels: Record<MeetingCaptureStatus, string> = {
        creating: 'Creating',
        joining: 'Joining',
    ...
    return props.captureStatus ? labels[props.captureStatus] : '';
});
...
const micPermissionTooltip = computed(() => {
    switch (micPermissionStatus.value) {
        ...
        case 'denied':
            return 'Microphone access denied - check System Preferences';
        case 'not-determined':
            return 'Click to request microphone permission';
        case 'restricted':
            return 'Microphone access restricted by system policy';
        default:
            return 'Click to check microphone permission status';
    }
});
...
const screenPermissionTooltip = computed(() => {
    switch (screenPermissionStatus.value) {
        case 'authorized':
            return 'Screen capture access granted';
        case 'denied':
            return 'Screen capture access denied - check System Preferences';
        case 'not-determined':
            return 'Click to request screen capture permission';
        case 'restricted':
            return 'Screen capture access restricted by system policy';
        default:
            return 'Click to check screen capture permission status';
    }
});
...
const requestMicrophonePermission = async () => {
    if (!shouldRequestLocalPermissions(props.captureMode)) return;
