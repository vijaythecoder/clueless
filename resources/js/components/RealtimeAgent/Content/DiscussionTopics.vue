<template>
    <div class="flex flex-col rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div
            v-else
            class="scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent flex min-h-0 flex-1 flex-wrap content-start gap-2 overflow-y-auto"
        >
            <div
                v-for="topic in topics"
                :key="topic.id"
                :class="['animate-fadeIn inline-flex h-6 items-center rounded-full px-2 text-xs font-medium', topicSentimentClass(topic.sentiment)]"
            >
