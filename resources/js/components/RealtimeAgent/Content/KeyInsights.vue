<template>
    <div class="flex h-full flex-col overflow-hidden rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div
            v-else
            class="scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent min-h-0 flex-1 space-y-2 overflow-y-auto"
        >
            <div v-for="insight in recentInsights" :key="insight.id" class="animate-fadeIn flex items-start gap-2">
                <div class="flex items-start gap-2">
                    <span :class="['inline-flex items-center rounded-full px-2 py-1 text-xs font-medium', insightTypeClass(insight.type)]">
